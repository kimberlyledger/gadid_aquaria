...
asv_table_with_sample_type <- metadata %>%
  filter(MiSeq_run == "B") %>%
  dplyr::select(Sample_ID, sample_type, project) %>%
  left_join(asv_table, by = "Sample_ID") %>%
  filter(project == "aquaria")
```

remove asv columns that have no reads 
```{r}
# Identify columns with sums not equal to 0
non_zero_sum_cols <- colSums(asv_table_with_sample_type[,4:382]) != 0

# Subset the dataframe to keep only columns with non-zero sums
asv_table_with_sample_type <- asv_table_with_sample_type[, non_zero_sum_cols]
```
...
asv_table_with_sample_type %>%
  pivot_longer(cols = c(4:145), names_to = "ASV", values_to = "reads") %>%
...
replicates_removed_2 %>%
  pivot_longer(cols = c(4:49), names_to = "ASV", values_to = "count") %>%
...
samples_removed_2 <- replicates_removed_2 %>%
  #filter(sample_type == "sample") %>%
  select(Sample_ID)
write.csv(samples_removed_2, "20230921_decontamination_step3.csv")
```

any ASV's with no reads? 
```{r}
asvs_toremove <- asv_table_filter3 %>%
  group_by(ASV) %>%
  summarise(total = sum(reads)) %>%
  filter(total == 0)
```
...
asv_table_filter3 <- asv_table_filter3 %>%
  filter(!ASV %in% asvs_toremove$ASV)
```
...
temp.df <- occu_df_subset
  ...
  myHierarchicalStanResults_20240313 <- tidy(tibble(as.data.frame(myHierarchicalModel)))
  
  write_rds(myHierarchicalStanResults_20240313, "20230921_MiSeqrun_occupancy_output_20240313.rds")
```


```{r}
myHierarchicalStanResults <- read_rds("20230921_MiSeqrun_occupancy_output_20240313.rds")
...
```{r}
#remove_5 <- c("e02528-A", "e02565-C", "e02593-A", "e02538-B")
...
length(unique(asv_table_filter4$ASV))
```
how many replicates do we have in our post-decontamination dataset 
```{r}
...
```{r}
#write.csv(asv_table_filter4, "20230921_decontamination_output_20240313.csv", row.names = FALSE)
```
