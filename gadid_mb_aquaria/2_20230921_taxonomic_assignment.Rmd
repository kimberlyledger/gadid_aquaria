...
```{r}
decontam_reads <- read.csv("/home/kimberly.ledger/gadid_aquaria/gadid_mb_aquaria/20230921_decontamination_output_20231030.csv") %>%
...
```{r}
decontam_asvs[51,] <- c("ASV22", "Gadus chalcogrammus", "species")
decontam_asvs[52,] <- c("ASV23", "Gadus chalcogrammus", "species")
decontam_asvs[53,] <- c("ASV37", "Gadus chalcogrammus", "species")
decontam_asvs[54,] <- c("ASV47", "Gadus macrocephalus", "species")
decontam_asvs[55,] <- c("ASV156", "Boreogadus saida", "species")
decontam_asvs[56,] <- c("ASV335", "Gadus chalcogrammus", "species")
```
...
```{r}
#write.csv(join, "/home/kimberly.ledger/gadid_metabarcoding/aquaria_DBO/20230921_gadid_aquariaDBO/20230921_taxon_reads.csv")
```
...
length(unique(aquaria_only$Sample_ID))
length(unique(aquaria_only$taxon))
```
