...
okay, just to be clear. subtracting the max proportion of ASVs found in the positive controls from the environmental samples and negative controls accounts for tag-jumping. 
...
what reads were removed? 
```{r}
tempB %>%
  #filter(PCR_blank > 0) %>%
  #filter(extraction_blank > 0) #%>%  #two ASVs had reads in extraction blanks
  #filter(field_blank > 0) %>% #three ASVs had reads in field blanks 
  filter(positive > 0) #three ASVs had reads in positives 
```

remove ASVs with no reads in aquaria samples from the data frame 
```{r}
...
```{r}
length(unique(asv_table_filter1.5$ASV)) 
```

calculate the maximum number of reads in an ASV to still show up in an extraction or PCR control
**excluding e02419 because there are clearly more reads in that negative than there should be... 
```{r}
reads_to_remove_per_ASV <- asv_table_filter1.5 %>%
  filter(Sample_ID != "e02419-A") %>%
  filter(Sample_ID != "e02419-B") %>%
  filter(Sample_ID != "e02419-C") %>%
...
samples_removed_2 <- replicates_removed_2 %>%
  #filter(sample_type == "sample") %>%
  select(Sample_ID)
#write.csv(samples_removed_2, "20230918_decontamination_step3.csv")
```
...
temp.df <- occu_df_subset
  ...
  #write_rds(myHierarchicalStanResults, "20230918_MiSeqrun_occupancy_output_20231030.rds")
```


```{r}
myHierarchicalStanResults <- read_rds("~/gadid_aquaria/gadid_mb_aquaria/20230918_MiSeqrun_occupancy_output_20231030.rds")
...
make a plot to see what the occupancy threshold should be 
```{r}
hist(site.asv_occupancy_probs$max_Occupancy_prob)
```
...
any ASV's with no reads? 
```{r}
asvs_noreads <- asv_table_filter4 %>%
  group_by(ASV) %>%
  summarise(total = sum(reads)) %>%
  filter(total == 0)
```
...
length(unique(asv_table_filter4$Sample_ID)) 
```
...
asv_table_filter4 %>%
  ...
  #filter(nrep == 2)  # there are two
  filter(nrep == 1) # there are two
```

can't run this test on extraction with only one rep
```{r}
temp_table <- asv_table_filter4 %>%
  filter(extraction_ID != "e02165") %>%
  filter(extraction_ID != "e02427") %>%
  filter(extraction_ID != "e02398")
```
good to go. 
...
write.csv(to_discard, "20230918_pcr_outliers.csv")
...
```{r}
#write.csv(asv_table_filter4, "20230918_decontamination_output_20240313.csv", row.names = FALSE)
```
