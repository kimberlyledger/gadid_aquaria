...
ggsave("figures/pairwise_dissimilarity.png", width = 6, height = 4, dpi = 300)
```
...
```{r}
#write.csv(mydata, "aquaria_taxonreads_20240313.csv")
```


### adding this comparison to illustrate common practices

look at the relationship between total metabarcoding reads and species biomass - just for illustration 
```{r}
...
calculate average number of reads for each species from a specific tank experiment.  

```{r}
tanks <- aquaria_data %>%
  group_by(site, taxon) %>%
  summarise(avg_reads = mean(total_reads),
            se_reads = sd(total_reads)/ sqrt(length(total_reads)),
            lower = avg_reads - 1.96 * se_reads,
            upper = avg_reads + 1.96 * se_reads) %>%
  dplyr::rename(Species = taxon) %>%
  dplyr::rename(tank_ID = site) %>%
...
okay, now let's plot this against what we know is the true biomass proportions
...
metadata <-  read.csv("/home/kimberly.ledger/gadid_aquaria/gadid_mb_aquaria/TankData_MBmodified.csv") %>% 
  ...
  dplyr::select(!sampleID) %>% #remove sample id because estimates are at the tank-level
  unique() %>%
  dplyr::rename(tank_ID = groupID) %>%
...
all_long <- metadata %>%
  ...
  left_join(tanks, by = c("tank_ID", "Species"))
```
...
read_count_fig <- all_long %>%
  ggplot() + 
  geom_errorbar(aes(x=Fbiomass,ymin=lower,ymax=upper,color=Species),width=0, size = .5) +
  geom_point(aes(x=Fbiomass,y=avg_reads,color=Species), size = 2) +
  scale_color_manual(values = mypal) + 
  theme_classic() + 
  labs(x = "proportional biomass",
       y = "average eDNA read count") +
...
```{r}
ggsave("figures/read_count.png", plot = read_count_fig, width = 8.5, height = 6, dpi = 300)
```


what about true species biomass
```{r}
read_count_biomass_fig <- all_long %>%
  pivot_longer(cols = c(15:17), names_to = "Species2", values_to = "biomass") %>%
  mutate(Species2 = ifelse(Species2 == "AC_biomass", "Arctic cod", Species2)) %>%
  mutate(Species2 = ifelse(Species2 == "PC_biomass", "Pacific cod", Species2)) %>%
  mutate(Species2 = ifelse(Species2 == "WP_biomass", "Walleye pollock", Species2)) %>%
  filter(Species == Species2) %>%
  ggplot() + 
  geom_errorbar(aes(x=biomass,ymin=lower,ymax=upper,color=Species),width=0, size = .5) +
  geom_point(aes(x=biomass,y=avg_reads,color=Species), size = 2) +
  scale_color_manual(values = mypal) + 
  theme_classic() + 
  ...
  theme(axis.text = element_text(size = 12),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 14))
read_count_biomass_fig 
```


```{r}
ggsave("figures/read_count_biomass.png", plot = read_count_biomass_fig, width = 8.5, height = 6, dpi = 300)
```
