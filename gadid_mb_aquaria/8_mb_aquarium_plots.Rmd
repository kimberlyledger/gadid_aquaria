...
metadata <- metadata %>%
  ...
  mutate(AC_ASM = (AC_biomass^0.65)) %>%   ### this is a simplification of ((total AC biomass^0.65)/N of AC) * N of AC
  mutate(PC_ASM = (PC_biomass^0.65)) %>%
  mutate(WP_ASM = (WP_biomass^0.65)) %>%
...
all_long_abundance <- metadata %>%
  pivot_longer(cols = c(36:38), names_to = "Species", values_to = "F_Abundance") %>%
...
all_long_ASM <- metadata %>%
  pivot_longer(cols = c(40:42), names_to = "Species", values_to = "F_ASM") %>%
...
fig_mock_biomass_by_spp_coloredtanks <- all_long_biomass %>%
  ...
  mutate(Species = ifelse(Species == "PC", "Pacific cod", Species)) %>%
  mutate(Species = ifelse(Species == "WP", "Walleye pollock", Species)) %>%
  mutate(to_color = ifelse(tank_ID %in% c("3", "46", "2", "10", "71", "80", "89"), tank_ID, NA)) %>%
  ggplot() + 
  ...
  #geom_smooth(aes(x=Fbiomass,y=mock.Mean,color=Species), method = "lm", formula = y ~ x - 1, se = FALSE) +
  #geom_smooth(aes(x=Fbiomass,y=mock.Mean), method = "lm", formula = y ~ x, se = FALSE, color = "light grey") +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")  + 
  facet_wrap(~Species) +
  #scale_color_manual(values = mypal) + 
  theme_bw() + 
...
```{r}
#ggsave("figures/biomass_by_tank.png", plot = fig_mock_biomass_by_spp_coloredtanks, width = 8.5, height = 3.5, dpi = 300)
```
...
```{r}
AIC(mock_mod_ac_biomass, mock_mod_ac_ASM, mock_mod_ac_abund)
AIC(mock_mod_pc_biomass, mock_mod_pc_ASM, mock_mod_pc_abund)
AIC(mock_mod_wp_biomass, mock_mod_wp_ASM, mock_mod_wp_abund)
```

```{r}
AIC(raw_mod_ac_biomass, raw_mod_ac_ASM, raw_mod_ac_abund)
AIC(raw_mod_pc_biomass, raw_mod_pc_ASM, raw_mod_pc_abund)
AIC(raw_mod_wp_biomass, raw_mod_wp_ASM, raw_mod_wp_abund)
```
