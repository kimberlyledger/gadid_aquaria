...
# Make plots
BREAKS <- c(0.0,0.05,0.10,0.25,0.50)
x.labs <- c("Uncorrected","Corrected")                          ## not sure what this is designating later on 
x.at   <- c(1,2)
...
let's visualize as scatterplots to match how we look at the tank experiment output
...
raw_plot <- all.dat %>%
  ...
  geom_smooth(aes(x=true.prop,y=raw.mean,color=Species), method = "lm", formula = y ~ x - 1, se = FALSE) +
...
mock_plot <- all.dat %>%
  ...
  geom_smooth(aes(x=true.prop,y=mock1.mean,color=Species), method = "lm", formula = y ~ x - 1, se = FALSE) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")  + 
  scale_color_manual(values = my_colors) + 
  theme_classic() + 
    labs(x = "true proportion",
...
now let's calculate slopes/r2 for those models (to report in supplementary?)
```{r}
arctic <- all.dat %>%
...
arctic_raw <- lm(raw.mean ~ true.prop - 1, data = arctic)
summary(arctic_raw)
arctic_mock <- lm(mock1.mean ~ true.prop - 1, data = arctic)
summary(arctic_mock)

saffron_raw <- lm(raw.mean ~ true.prop - 1, data = saffron)
summary(saffron_raw)
saffron_mock <- lm(mock1.mean ~ true.prop - 1, data = saffron)
summary(saffron_mock)

pollock_raw <- lm(raw.mean ~ true.prop - 1, data = pollock)
summary(pollock_raw)
pollock_mock <- lm(mock1.mean ~ true.prop - 1, data = pollock)
summary(pollock_mock)

pacific_raw <- lm(raw.mean ~ true.prop - 1, data = pacific)
summary(pacific_raw)
pacific_mock <- lm(mock1.mean ~ true.prop - 1, data = pacific)
summary(pacific_mock)

tomcod_raw <- lm(raw.mean ~ true.prop - 1, data = tomcod)
summary(tomcod_raw)
tomcod_mock <- lm(mock1.mean ~ true.prop - 1, data = tomcod)
summary(tomcod_mock)

polar_raw <- lm(raw.mean ~ true.prop - 1, data = polar)
summary(polar_raw)
polar_mock <- lm(mock1.mean ~ true.prop - 1, data = polar)
summary(polar_mock)
```
