...
combined %>%
  ...
  theme_classic() + 
  #geom_smooth(aes(x=biomass,y=Conc), formula = y ~ x - 1, color = "black", linetype = "dashed", method = "lm", se = FALSE) +
  geom_abline(slope = coef(mod3)[1], intercept = 0, color = "black", linetype = "dashed") +
...
combined %>%
  ...
  #geom_point(aes(x=biomass, y=Conc, label = site_name)) +
  theme_classic() +
...
combined %>%
  ...
  theme_classic() +
  scale_color_manual(values = mypal) + 
  geom_smooth(aes(x=biomass,y=Conc, color = Species), formula = y ~ x - 1, linetype = "dashed", method = "lm", se = FALSE) +
  #scale_color_gradient2(low = "darkgreen", mid = "darkgrey", high = "purple", midpoint = 0.5, limits = c(0, 1), breaks = c(0.25, 0.5, 0.75)) +
  labs(x = "biomass (grams)",
       y = "log10 DNA copy number")
