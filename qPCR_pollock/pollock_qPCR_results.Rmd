...
arctic cod - log10 transformed x and y 
```{r}
ac <- combined %>%
  filter(Species == "Arctic cod") %>%
  filter(biomass > 0)

10^(mean(ac$Conc_mean))
10^(median(ac$Conc_mean))
10^(quantile(ac$Conc_mean, probs = 0.025))
10^(quantile(ac$Conc_mean, probs = 0.975))

#ac_lm1 <- lm(Conc_mean ~ log10(biomass)-1, data = ac)
#summary(ac_lm1)

ac_lm2 <- lm(Conc_mean ~ log10(biomass), data = ac)
summary(ac_lm2)
...
#   geom_point(aes(x=log10(biomass), y=Conc_mean, color = Species), cex = 2) + 
#   geom_errorbar(aes(x=log10(biomass), y=Conc_mean, ymin = Conc_q.025, ymax = Conc_q.975, color = Species), width = 0) +
#   geom_abline(slope = coef(ac_lm1)[1], intercept = 0, color = "black", linetype = "dashed") +
#   theme_classic() + 
...
ac_plot <- ac %>%  
  ...
  geom_point(aes(x=log10(biomass), y=Conc_mean, color = Species), cex = 2) + 
  geom_errorbar(aes(x=log10(biomass), y=Conc_mean, ymin = Conc_q.025, ymax = Conc_q.975, color = Species), width = 0) +
  geom_abline(slope = coef(ac_lm2)[2], intercept = coef(ac_lm2)[1], color = "black", linetype = "dashed") +
  theme_classic() + 
  scale_color_manual(values = "#6a6599") + 
  labs(x = "log10(biomass)",
       y = "log10(DNA copy number)",
...
pacific cod - log10 transformed x and y 
```{r}
pc <- combined %>%
...
10^(mean(pc$Conc_mean))
10^(median(pc$Conc_mean))
10^(quantile(pc$Conc_mean, probs = 0.025))
10^(quantile(pc$Conc_mean, probs = 0.975))


#pc_lm1 <- lm(Conc_mean ~ log10(biomass)-1, data = pc)
#summary(pc_lm1)

pc_lm2 <- lm(Conc_mean ~ log10(biomass), data = pc)
summary(pc_lm2)
...
#   geom_point(aes(x=log10(biomass), y=Conc_mean, color = Species), cex = 2) + 
#   geom_errorbar(aes(x=log10(biomass), y=Conc_mean, ymin = Conc_q.025, ymax = Conc_q.975, color = Species), width = 0) +
#   geom_abline(slope = coef(pc_lm1)[1], intercept = 0, color = "black", linetype = "dashed") +
#   theme_classic() + 
...
pc_plot <- pc %>%  
  ...
  geom_point(aes(x=log10(biomass), y=Conc_mean, color = Species), cex = 2) + 
  geom_errorbar(aes(x=log10(biomass), y=Conc_mean, ymin = Conc_q.025, ymax = Conc_q.975, color = Species), width = 0) +
  geom_abline(slope = coef(pc_lm2)[2], intercept = coef(pc_lm2)[1], color = "black", linetype = "dashed") +
  theme_classic() + 
  scale_color_manual(values = "#79af97") + 
  labs(x = "log10(biomass)",
       y = "log10(DNA copy number)",
...
```{r}
ggsave("my_figures/ac_pc_estimates.png", plot = combined_fig, width = 8, height = 4, dpi = 300)
```
...
ac <- combined %>%
  filter(Species == "Arctic cod") %>%
  filter(biomass > 0) %>%
  filter(WP_Fbiomass >= 0.25)   #should remove 2 tanks
...
ac_lm3 <- lm(Conc_mean ~ log10(biomass), data = ac)
summary(ac_lm3)
...
ac_plot_reduced <- ac %>%  
  ...
  geom_point(aes(x=log10(biomass), y=Conc_mean, color = Species), cex = 2) + 
  geom_errorbar(aes(x=log10(biomass), y=Conc_mean, ymin = Conc_q.025, ymax = Conc_q.975, color = Species), width = 0) +
  geom_abline(slope = coef(ac_lm3)[2], intercept = coef(ac_lm3)[1], color = "black", linetype = "dashed") +
  theme_classic() + 
  scale_color_manual(values = "#6a6599") + 
  labs(x = "log10(biomass)",
       y = "log10(DNA copy number)",
...
pc_lm3 <- lm(Conc_mean ~ log10(biomass), data = pc)
summary(pc_lm3)
